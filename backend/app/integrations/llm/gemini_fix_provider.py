...
from app.integrations.llm.fix_prompts import (
    MAX_FIX_PROVIDER_INPUT_CHARACTERS,
    build_fix_generation_input,
)
...
class GeminiFixGenerator:
    """Gemini adapter for one bounded, structured fix suggestion."""
    ...
    async def generate_fix(
        ...
        if not supplied_indices or len(set(supplied_indices)) != len(supplied_indices):
            raise ValueError("fix provider context requires unique source segment indices")
        provider_input = build_fix_generation_input(requirement, transcript_segments)
        if len(provider_input) > MAX_FIX_PROVIDER_INPUT_CHARACTERS:
            raise LLMConfigurationError("The fix input exceeds its safe provider bound.")
        ...
        try:
            interaction = await self._client.aio.interactions.create(
                ...
                response_format={
                    ...
                    "schema": gemini_response_schema(FixProviderOutput),
                },
            ...
            raise LLMMalformedOutputError(
                "The fix provider returned malformed output."
            ) from error
        except errors.APIError as error:
            raise translate_api_error(error) from error
        except (TimeoutError, httpx.TimeoutException) as error:
            raise LLMProviderTimeoutError("The fix provider timed out.") from error
        except httpx.RequestError as error:
            raise LLMProviderUnavailableError(
                "The fix provider is unavailable."
            ) from error

        output_text = getattr(interaction, "output_text", None)
        if not isinstance(output_text, str) or not output_text.strip():
            raise LLMMalformedOutputError(
                "The fix provider returned no structured output."
            )
        try:
            output = FixProviderOutput.model_validate_json(output_text)
